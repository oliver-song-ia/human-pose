...
class TRTDinov3Backbone(nn.Module):
    ...
    def _init_engine(self):
        ...
        input_shape = tuple(self._engine.get_tensor_shape(self.input_name))
        output_shape = tuple(self._engine.get_tensor_shape(self.output_name))
        if any(dim < 0 for dim in input_shape + output_shape):
            raise RuntimeError(
                f"TensorRT backbone must have fixed shapes, got input={input_shape}, "
                f"output={output_shape}")
        self.input_shape = input_shape
        self.output_shape = output_shape
        self.embed_dim = self.embed_dims = output_shape[1]
        self.output_size = output_shape[-2:]

        # Shapes are fixed, so allocate the I/O buffers once and bind their
        # addresses once.  Doing it per call (torch.empty for the output plus
        # .to(fp16).contiguous() for the input) cost ~29 ms per frame on a
        # Jetson -- as much as half the engine's own runtime -- because the
        # allocator and the H2D staging run on the CPU, which is the scarce
        # resource there rather than the GPU.
        import torch as _torch
        dev = _torch.device("cuda")
        self._in_buf = _torch.empty(self.input_shape, device=dev, dtype=_torch.float16)
        self._out_buf = _torch.empty(self.output_shape, device=dev, dtype=_torch.float16)
        self._context.set_tensor_address(self.input_name, self._in_buf.data_ptr())
        self._context.set_tensor_address(self.output_name, self._out_buf.data_ptr())
    ...
    def forward(self, x, extra_embed=None):
        ...
        if tuple(x.shape) != self.input_shape:
            raise ValueError(
                f"TensorRT backbone expects {self.input_shape}, got {tuple(x.shape)}")

        # Copy into the pre-bound input buffer (handles dtype and layout in one
        # kernel); the addresses were set once at load time.
        self._in_buf.copy_(x, non_blocking=True)
        ...
        # Clone so a caller holding the result is not overwritten by the next
        # frame, which would otherwise silently share this persistent buffer.
