...
import cv2
import numpy as np
# detectron2 is needed by exactly one function in this file
# (parse_pose_metainfo, below).  Importing it at module scope forces the
# whole dependency on every caller -- including a --no-overlay worker that
# never draws a skeleton -- and detectron2 has no aarch64 wheel.  Import
# it where it is actually used instead.
