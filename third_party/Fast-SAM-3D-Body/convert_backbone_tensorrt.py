...
# Model config
IMAGE_SIZE = (384, 384)  # H, W; overridden by --image-size
EMBED_DIM = 1280  # dinov3_vith16plus
PATCH_SIZE = 16
OUTPUT_SIZE = (24, 24)  # IMAGE_SIZE / PATCH_SIZE
...
def step1_export_onnx(backbone, batch_sizes=[1]):
    """Export backbone to ONNX for the fixed real-time shape."""
    print("=" * 60)
    ...
    # Export a fixed batch=1 graph.  The live worker only submits one person,
    # and a static shape gives TensorRT more optimization freedom.
...
def step2_convert_tensorrt(batch_sizes=[1]):
    ...
    print(f"  Fixed input shape: [1, 3, {IMAGE_SIZE[0]}, {IMAGE_SIZE[1]}]")
...
def main():
    ...
    parser.add_argument("--batch_sizes", type=str, default="1", help="Must be 1 for the fixed real-time engine")
    parser.add_argument("--image-size", type=int, default=384, choices=(384, 448, 512),
                        help="Fixed square backbone input size (live default: 384)")

    args = parser.parse_args()

    batch_sizes = [int(x) for x in args.batch_sizes.split(",")]
    if batch_sizes != [1]:
        parser.error("the optimized real-time engine requires --batch_sizes 1")

    global IMAGE_SIZE, OUTPUT_SIZE, ONNX_PATH, TRT_PATH, TRT_PATH_FP16
    IMAGE_SIZE = (args.image_size, args.image_size)
    OUTPUT_SIZE = (args.image_size // PATCH_SIZE, args.image_size // PATCH_SIZE)
    suffix = "" if args.image_size == 384 else f"_{args.image_size}"
    ONNX_PATH = os.path.join(TRT_OUTPUT_DIR, f"backbone_dinov3{suffix}.onnx")
    TRT_PATH_FP16 = os.path.join(TRT_OUTPUT_DIR, f"backbone_dinov3_fp16{suffix}.engine")
    TRT_PATH = TRT_PATH_FP16
    os.environ["IMG_SIZE"] = str(args.image_size)
